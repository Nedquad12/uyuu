...
class RateLimiter:
    def __init__(self):
        ...
        # Konfigurasi rate limiting untuk VIP
        self.VIP_MAX_REQUESTS = 2
        self.VIP_COOLDOWN_MINUTES = 30  # 0.5 jam
    ...
    def check_rate_limit(self, user_id: int) -> Tuple[bool, str]:
        ...
        """
        # Non-VIP users tidak diizinkan akses
        if not self.is_vip(user_id):
            return False, "🚫 Akses ditolak! Hanya VIP yang bisa menggunakan bot ini.\n\n💎 Ketik /id dan berikan id pada admin agar bisa akses bot"
        ...
        if user_id not in self.user_limits:
            reset_time = current_time + timedelta(minutes=self.VIP_COOLDOWN_MINUTES)
            self.user_limits[user_id] = {
                'count': 1,
                'reset_time': reset_time
            }
            return True, f"✅ Request berhasil! Sisa quota: {self.VIP_MAX_REQUESTS - 1}/{self.VIP_MAX_REQUESTS}"
        ...
        if user_data['count'] >= self.VIP_MAX_REQUESTS:
            ...
            return False, f"⏳ Limit quota tercapai! Tunggu {time_str} lagi untuk request berikutnya."
        
        # Masih bisa request, tambah counter
        user_data['count'] += 1
        remaining_requests = self.VIP_MAX_REQUESTS - user_data['count']
        
        if remaining_requests > 0:
            return True, f"✅ Request berhasil! Sisa quota: {remaining_requests}/{self.VIP_MAX_REQUESTS}"
        else:
            return True, f"✅ Request berhasil! Quota habis, reset dalam {self.VIP_COOLDOWN_MINUTES} menit."

    def get_user_status(self, user_id: int) -> str:
        """Get status lengkap untuk user tertentu"""
        if not self.is_vip(user_id):
            return "🚫 Status: Non-VIP (Tidak ada akses)\n\n💎 Ketik /id dan berikan id pada admin agar bisa akses bot"
            
        self._cleanup_expired_entries()
        
        if user_id not in self.user_limits:
            return f"📊 Status VIP: {self.VIP_MAX_REQUESTS}/{self.VIP_MAX_REQUESTS} request tersisa"
        
        user_data = self.user_limits[user_id]
        current_time = datetime.now()
        
        if current_time >= user_data['reset_time']:
            return f"📊 Status VIP: {self.VIP_MAX_REQUESTS}/{self.VIP_MAX_REQUESTS} request tersisa (reset)"
        
        remaining = self.VIP_MAX_REQUESTS - user_data['count']
        time_left = user_data['reset_time'] - current_time
        hours_left = time_left.total_seconds() / 3600
        minutes_left = (time_left.total_seconds() % 3600) / 60
        ...
        return f"📊 Status VIP: {remaining}/{self.VIP_MAX_REQUESTS} request tersisa, reset dalam {time_str}"
...
def with_rate_limit(func):
    ...
    async def wrapper(update, context):
        ...
        try:
            ...
            # Jika ada error, kembalikan quota (rollback) - untuk VIP
            if rate_limiter.is_vip(user_id) and user_id in rate_limiter.user_limits:
