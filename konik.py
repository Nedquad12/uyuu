...
class TelegramStockDataViewer:
            ...
            for file_path in excel_files:
                ...
                # Check if required columns exist
                    required_cols = ['Kode Saham', 'Penutupan', 'Volume', 'Frekuensi', 
                                   'Foreign Buy', 'Foreign Sell', 'Listed Share']
    ...
    def get_watchlist_stocks(self, cap_filter=None):
        ...
        # Calculate additional metrics
            ...
            filtered_stocks['Market Cap'] = filtered_stocks['Penutupan'] * filtered_stocks['Listed Share']
...
Change: {mv_arrow} ${mv_change:+,.0f} ({mv_change_pct:+.2f}%)"""
    ...
    def format_watchlist_response(self, stocks, cap_filter=None):
        ...
        response += f"\n\n🔍 Criteria: Volume & Frekuensi ≥70% above average\n"
...
/chart - Membuat Chart
/wl - Watchlist saham aktif (Volume & Frekuensi tinggi)
...
def watchlist_command(message):
    ...
    if viewer.watchlist_data is None:
        bot.reply_to(message, "❌ No watchlist data loaded. Please ensure Excel files are in the data folder.")
        return
    ...
    bot.reply_to(message, "📊 Select market cap filter for watchlist:", reply_markup=markup)
