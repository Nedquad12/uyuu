...
class Blackrock:
    ...
    def create_blackrock_chart(self, region, ticker):
        ...
        fig, axes = plt.subplots(2, 1, figsize=(12, 10)) if listed_shares else (None, None)
        ...
        # Plot 2: % kepemilikan vs Listed Shares
        if listed_shares and ax2 is not None:
            ownership_pct = (grouped[qty_col] / listed_shares) * 100
            ax2.plot(grouped.index, ownership_pct, marker='s', linewidth=2, color='green')
            ax2.set_title(f'% Kepemilikan {company_name} vs Listed Shares', fontsize=12, fontweight='bold')
            ax2.set_xlabel('Date', fontsize=11)
            ax2.set_ylabel('Kepemilikan (%)', fontsize=11)
    ...
    def generate_movement_caption(self, grouped_data, ticker, region, listed_shares=None):
        ...
        # % kepemilikan vs Listed Shares
        if listed_shares and listed_shares > 0:
            own_latest   = (latest[qty_col]   / listed_shares) * 100
            own_previous = (previous[qty_col] / listed_shares) * 100
            own_change   = own_latest - own_previous
            own_arrow    = "🔺" if own_change > 0 else "🔻" if own_change < 0 else "➡️"
            caption += f"\n📌 Kepemilikan vs Listed Shares ({listed_shares:,.0f}):\n"
            caption += f"   Current : {own_latest:.4f}%\n"
            caption += f"   Previous: {own_previous:.4f}%\n"
            caption += f"   Change  : {own_arrow} {own_change:+.4f}%\n"
