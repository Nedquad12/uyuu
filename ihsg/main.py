...
from blackrock import blackrock_significant_movements, blackrock_indonesia, spdr_command
...
from index_reader import convert_all_index_files
from wti_command import build_wti_index_cache, register_wti_handler
from predict_command import register_predict_handler
from api_server import start_api_server
from repo_command import register_repo_handler
...
from ma_command import register_ma_handlers, build_ma_screener_cache
from bs_command import register_bs_handler
...
def do_reload() -> str:
    ...
    build_kons_cache()
    build_atr_cache()
    ma_summary = build_ma_screener_cache(json_dir=OUTPUT_DIR)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "`/atr XXXX` — Melihat data atr sebuah saham\\.\n"
        "`/ma` — Analisa saham dengan ma tertentu\\.\n"
        "\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "🔍 *Screener*\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        "`/bi XXXX` — Kepemilikan BlackRock Indonesia beserta % vs listed shares\\.\n"
        "`/spdr XXXX` — Kepemilikan SPDR beserta % vs listed shares\\.\n"
        ...
        "📉 *Korelasi & Prediksi*\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "`/wtii XXXX` — Lihat seberapa sering saham ikut arah IHSG Composite dalam 90 hari terakhir\\.\n"
        "`/wtii XXXX INDEX` — Bandingkan vs indeks lain \\(LQ45, IDXFINANCE, dll\\)\\.\n"
        "`/predict XXXX` — Prediksi ML 3 hari ke depan: arah harga \\+ apakah ikut Composite\\.\n"
        "`/predict XXXX INDEX` — Prediksi dengan benchmark indeks pilihan\\.\n"
        "\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "ℹ️ *Lainnya*\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
...
async def cmd_reload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔄 Reload dimulai, bot tetap aktif...")

    # Jalankan reload di background task — bot ga nunggu selesai
    async def _bg():
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, do_reload)
        await run_screener(context.bot)
...
def main():
    ...
    build_wti_index_cache()
    build_kons_cache()
    build_atr_cache()
    build_ma_screener_cache(json_dir=OUTPUT_DIR)
    start_api_server()
    ...
    app.add_handler(CommandHandler("spdr",  spdr_command)) 
    ...
    register_wti_handler(app)
    register_predict_handler(app)
    register_repo_handler(app)
    register_kons_handler(app)
    register_atr_handler(app)
    register_bs_handler(app)
    register_ma_handlers(app, json_dir=OUTPUT_DIR)
