"""
kons_command.py — Handler Telegram untuk /kons TICKER [N]
...
Mode:
  /kons BBCA      → snapshot data terbaru
  /kons BBCA 2    → perbandingan file terbaru vs 2 bulan lalu
                    (fallback ke snapshot jika file tidak cukup)
...
# Nama lengkap Indonesia untuk caption
FULL_LABELS = {
    "BANK":                                                    "Bank",
    "GOVERNMENT":                                              "Pemerintah",
    "PRIVATE EQUITY":                                          "Private Equity",
    "TRUSTEE BANK":                                            "Bank Kustodian",
    ...
    "CAPITAL MARKET SUPPORTING INSTITUTIONS AND PROFESSIONS":  "Lembaga Penunjang Pasar Modal",
    "COMMANDITAIRE VENNOOTSCHAP (CV) OR LIMITED PARTNERSHIP":  "CV / Persekutuan Komanditer",
    "FIRM":                                                    "Perusahaan",
    ...
    "CORPORATE":                                               "Korporasi Swasta",
    "ASSOCIATION/SOCIAL ORGANIZATIONS":                        "Asosiasi / Organisasi Sosial",
    ...
    "MUTUAL FUNDS (MF)":                                       "Reksa Dana",
    "SECURITIES COMPANY (SC)":                                 "Perusahaan Sekuritas",
    "PENSION FUNDS (PF)":                                      "Dana Pensiun",
    "FINANCIAL INSTITUTIONAL (IB)":                            "Lembaga Keuangan / Bank",
    "INSURANCE (IS)":                                          "Asuransi",
    "FOUNDATION (FD)":                                         "Yayasan",
    "INDIVIDUAL (ID)":                                         "Investor Ritel",
}
...
def _make_single_chart(
    ...
    fig.suptitle(
        f"Klasifikasi Investor — {ticker.upper()}{suffix}\n{date_label}",
    ...
    ax.set_title("Jumlah Scripless Shares per Kategori", fontsize=11, fontweight="bold")
    ax.set_ylabel("Lembar Saham")
...
def _make_snapshot_charts(
    ...
    if not sorted_entries:
        raise ValueError(f"Semua kategori bernilai 0 untuk {ticker}")
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        caption = (
            f"📊 <b>Klasifikasi Investor — {ticker}</b>"
...
def _make_comparison_charts(
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        fig.patch.set_facecolor("white")
        fig.suptitle(
            f"Perubahan Klasifikasi Investor — {ticker.upper()}{suffix}\n"
            f"{date_old}  →  {date_new}",
            fontsize=13, fontweight="bold", y=1.02,
        )
        ...
        caption = (
            f"📊 <b>Perubahan Klasifikasi — {ticker}</b>"
            + (f" ({idx}/{total})" if total > 1 else "")
            + f"\n\n{_build_caption(cols)}"
        )
        result.append((buf, caption))
...
def create_kons_charts(
    ...
    if n == 0 or len(files) == 1:
        ...
        if n > 0 and len(files) == 1:
            fallback_msg = "⚠️ Data pembanding belum tersedia (baru 1 bulan). Menampilkan snapshot terbaru.\n\n"
    ...
    if n >= len(files):
        ...
        fallback_msg = (
            f"⚠️ Hanya tersedia {len(files)} bulan data. "
            f"Menampilkan perbandingan {actual_n} bulan.\n\n"
        )
    else:
        actual_n     = n
        fallback_msg = ""
...
async def cmd_kons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args:
        await update.message.reply_text(
            "⚠️ Gunakan: <code>/kons KODE [N]</code>\n"
            "Contoh: <code>/kons BBCA</code> atau <code>/kons BBCA 2</code>",
    ...
    msg = await update.message.reply_text(
        f"⏳ Membuat chart klasifikasi <b>{ticker}</b>…",
        parse_mode=ParseMode.HTML,
    )
    ...
    try:
        ...
        if not charts:
            await msg.edit_text(
                f"❌ Data <b>{ticker}</b> tidak ditemukan.",
                parse_mode=ParseMode.HTML,
            )
            return
...
def register_kons_handler(app) -> None:
    """Daftarkan handler /kons ke Application. Panggil dari main.py."""
