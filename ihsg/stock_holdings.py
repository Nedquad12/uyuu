...
class HoldingsManager:
    ...
    def search_by_investor(self, name: str):
        ...
        # Cari semua saham yang dimiliki orang ini
        ...
        return [(cap, img) for _, cap, img in stock_list]   # return (caption, img_path)
    ...
    def _generate_combined_image(self, df: pd.DataFrame, ticker: str, company_name: str):
        ...
        # === GAMBAR BESAR - TABLE LEBIH GEBU & PIE LEBIH KECIL ===
        fig = plt.figure(figsize=(21, 13.5))
        gs = fig.add_gridspec(1, 2, width_ratios=[1.45, 0.55], wspace=0.04)   # TABLE LEBIH BESAR

        fig.suptitle(f"KEPEMILIKAN SAHAM UTAMA\n{ticker} ({company_name})", 
                     fontsize=17, fontweight='bold', y=0.96)

        # --- TABEL KIRI (FULL HEIGHT, NO KOSONG) ---
        ax_table = fig.add_subplot(gs[0])
        ax_table.axis('off')

        headers = ['Investor Name', 'Type', 'L/F', 'Domicile', 'Scripless', 'Scrip', 'Total Shares', '%']
        table_data = []
        max_rows = 17   # naik sedikit biar lebih banyak

        for _, row in df.head(max_rows).iterrows():
            inv_name = str(row['INVESTOR_NAME'])[:34]      # nama lebih panjang
        ...
        col_widths = [0.39, 0.06, 0.05, 0.10, 0.11, 0.085, 0.135, 0.07]   # nama lebih lebar

        # BBOX BIAR TABLE FULL ISI SELURUH AREA (no kosong di bawah)
        table = ax_table.table(cellText=full_table, colWidths=col_widths,
                               bbox=[0.01, 0.02, 0.98, 0.96],   # full height
