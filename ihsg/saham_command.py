...
def _load_json_files(limit: int = 60) -> list[tuple[str, datetime]]:
    """Return sorted (path, date) list dari JSON_DIR, newest first.
    Format file: ddmmyy.json — array of objects.
    """
    ...
    for fname in os.listdir(JSON_DIR):
        ...
        try:
            ...
            result.append((os.path.join(JSON_DIR, fname), d))
        except ValueError:
            continue
        ...
        for fpath, date_obj in files:
            try:
                ...
                if not isinstance(data, list):
                    continue
                row = next((r for r in data if r.get('Kode Saham') == stock_code), None)
                if not row:
                    continue
                vol   = float(row.get('Volume',    0) or 0)
                price = float(row.get('Penutupan', 0) or 0)
    ...
    try:
        ...
        for fpath, date_obj in files:
            try:
                ...
                if not row:
                    continue
                buy  = float(row.get('Foreign Buy',  0) or 0)
                sell = float(row.get('Foreign Sell', 0) or 0)
                net  = float(row.get('Foreign Net',  buy - sell))
                foreign_data.append({'date': date_obj, 'foreign_buy': buy,
                                     'foreign_sell': sell, 'foreign_net': net})
            except Exception as e:
