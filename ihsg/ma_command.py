...
# ── Cache ──────────────────────────────────────────────────────────────────────
# Struktur:
#   _MA_CACHE[label] = list of (kode, price, ma_val, diff_pct)   → untuk /ma
#   _AM_CACHE        = list of (kode, price, mas_dict, max_diff) → untuk /am
#   _LM_CACHE        = list of (kode, price, mas_dict, max_diff) → untuk /lm
        ...
        try:
            ...
            for label in MA_LABELS:
                ...
                if abs(diff_pct) <= THRESHOLD_MA:
                    new_ma_cache[label].append((kode, price, ma_val, diff_pct))
            ...
            if all(mas[l] is not None for l in MA_LABELS):
                ...
                if all(diffs[l] > 0 for l in MA_LABELS):
                    ...
                    if max_diff <= THRESHOLD_AM:
                        new_am_cache.append((kode, price, {l: mas[l] for l in MA_LABELS}, diffs))
            ...
            if all(mas[l] is not None for l in MA_LABELS):
                ...
                if all(diffs[l] < 0 for l in MA_LABELS):
                    ...
                    if abs(max_diff) <= THRESHOLD_LM:
                        new_lm_cache.append((kode, price, {l: mas[l] for l in MA_LABELS}, diffs))

        except Exception as e:
            logger.debug("[MA] Skip %s: %s", kode, e)
            continue

    # Sort: /ma → diff_pct terkecil (paling dekat)
    for label in MA_LABELS:
        new_ma_cache[label].sort(key=lambda x: abs(x[3]))

    # Sort: /am → max_diff terkecil (paling ketat di atas MA)
    new_am_cache.sort(key=lambda x: max(x[3].values()))

    # Sort: /lm → abs max_diff terkecil (paling dekat di bawah MA)
    new_lm_cache.sort(key=lambda x: max(abs(v) for v in x[3].values()))
...
def _fmt_ma_list(items: list, label: int) -> str:
    """Format hasil /ma untuk MA tertentu."""
    period = MA_LABEL_TO_PERIOD[label]
    if not items:
        return f"Tidak ada saham yang berada dalam jarak {THRESHOLD_MA}% dari MA{label} (periode {period})."

    lines = [
        f"📊 SAHAM DEKAT MA{label} (periode {period}) — maks ±{THRESHOLD_MA}%",
        f"⏰ Cache: {_cache_time_str()}",
        f"{'='*46}",
        f"{'Kode':<6} {'Harga':>8} {'MA'+str(label):>8} {'Diff%':>8} {'Pos':>5}",
        f"{'-'*46}",
    ]
    for kode, price, ma_val, diff_pct in items:
        pos = "▲" if diff_pct > 0 else "▼"
        lines.append(
            f"{kode:<6} {price:>8,.0f} {ma_val:>8,.0f} {diff_pct:>+7.2f}% {pos:>5}"
        )
    lines.append(f"{'='*46}")
...
def _fmt_am_list(items: list) -> str:
    """Format hasil /am."""
    if not items:
        return f"Tidak ada saham yang di atas semua MA (20/50/100/200) dengan jarak ≤{THRESHOLD_AM}%."

    lines = [
        f"💚 ALL MA ATAS — Harga di atas MA20/50/100/200 (maks {THRESHOLD_AM}%)",
        f"⏰ Cache: {_cache_time_str()}",
        f"{'='*58}",
        f"{'Kode':<6} {'Harga':>8} {'MA20%':>7} {'MA50%':>7} {'MA100%':>8} {'MA200%':>8}",
        f"{'-'*58}",
    ]
    for kode, price, mas, diffs in items:
        lines.append(
            f"{kode:<6} {price:>8,.0f} "
            f"{diffs[20]:>+6.2f}% "
            f"{diffs[50]:>+6.2f}% "
...
def _fmt_lm_list(items: list) -> str:
    """Format hasil /lm."""
    if not items:
        return f"Tidak ada saham yang di bawah semua MA (20/50/100/200) dengan jarak ≤{THRESHOLD_LM}%."
...
async def cmd_ma(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📊 *Moving Average Screener*\n\n"
        f"Pilih periode MA untuk melihat saham yang berada dalam jarak ±{THRESHOLD_MA}% dari MA tersebut:\n\n"
...
async def ma_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle tombol pilihan MA periode."""
    ...
    # Split jika terlalu panjang (Telegram max 4096 char)
    if len(msg) <= 4000:
        ...
        # Kirim sebagai pesan baru jika terlalu panjang
