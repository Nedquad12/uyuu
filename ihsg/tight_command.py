...
Cache di-build saat startup / reload, bukan saat user memanggil command.
/vt  → daftar saham Very Tight  (cache langsung dikembalikan)
/t   → daftar saham Tight       (cache langsung dikembalikan)
"""
...
_cache: dict = {
    "vt":    [],
    "t":     [],
    "built": None,   # string kapan terakhir di-build
}


# ── Scanner (dipanggil dari do_reload, bukan dari handler) ────────────────────

def build_tight_cache(json_dir: str = JSON_DIR) -> tuple[int, int]:
    """
    Scan semua ticker dan simpan hasilnya ke _cache di RAM.
    Dipanggil oleh do_reload() di main.py setelah XLSX → JSON selesai.
...
def _format_table(title: str, results: list[dict], built: str) -> list[str]:
    """Kembalikan list pesan, dipecah per 30 baris agar < 4096 karakter."""
    if not results:
