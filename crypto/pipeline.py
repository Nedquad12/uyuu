...
def run(
    ...
    regime_msg = (
        f"🌡️ <b>{symbol}</b> — Regime Detected: <b>{regime}</b>\n"
    ...
    _notify(f"📊 <b>{symbol}</b> — Running Walk-Forward Validation...")
    ...
    _notify(f"🧠 <b>{symbol}</b> — Analisis AI (regime={regime})...")
    ai_result = ai_analyze(pred, wfv_result, train_result)
    ...
    ai_msg = (
        ...
        f"  Regime      : <b>{ai_result.get('regime', regime)}</b>\n"
    ...
    if PAPER_TRADING_MODE:
        _notify(f"📝 <b>{symbol}</b> — PAPER TRADE {ai_action} (simulasi)...")
