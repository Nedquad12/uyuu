...
# ------------------------------------------------------------------
# Filter pump/dump: skip koin yang sudah naik/turun berlebihan
# ------------------------------------------------------------------
_PUMP_DUMP_CANDLES   = 18    # lookback 18 candle terakhir
_PUMP_DUMP_THRESHOLD = 0.30  # 30% naik atau turun → skip


def _is_pump_or_dump(df) -> tuple[bool, float]:
    """
    Cek apakah koin sudah pump atau dump berlebihan dalam 18 candle terakhir.
    Bandingkan close candle ke-(-18) vs close candle terbaru.
    Return (True, pct_change) jika abs(pct) >= threshold, else (False, pct).
    """
    if len(df) < _PUMP_DUMP_CANDLES:
        return False, 0.0

    close_start = float(df["close"].iloc[-_PUMP_DUMP_CANDLES])
    close_end   = float(df["close"].iloc[-1])

    if close_start <= 0:
        return False, 0.0

    pct = (close_end - close_start) / close_start
    return abs(pct) >= _PUMP_DUMP_THRESHOLD, pct
...
def score_symbol(
    ...
    for attempt in range(max_retries):
        try:
            ...
            # ── Filter pump/dump: skip koin yang sudah naik/turun > 30% dalam 18 candle ──
