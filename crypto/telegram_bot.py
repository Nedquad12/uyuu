...
# ------------------------------------------------------------------
# /portofolio — Modal, posisi aktif, riwayat, PnL
# ------------------------------------------------------------------
@restricted
async def cmd_portofolio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        import time as _time
        from order.paper_executor import _load_positions, _load_history, get_available_balance
        from config import PAPER_BALANCE_USDT
        ...
        lines = [
            ...
            f"  Margin terpakai: <code>{used_margin:.2f} USDT</code>",
        ...
        if open_pos:
            lines.append(f"\n📂 <b>Posisi Aktif ({len(open_pos)})</b>")
            for p in open_pos:
                emoji  = "🟢" if p["side"] == "BUY" else "🔴"
                hold_h = (_time.time() - p["opened_at"]) / 3600
                lines.append(
                    f"  {emoji} <b>{p['symbol']}</b> ×{p['leverage']}\n"
                    f"     Entry <code>{p['entry_price']}</code>  "
                    f"Margin <code>{p['margin_used']} USDT</code>  "
                    f"Hold {hold_h:.1f}h"
                )
        else:
        ...
        if history:
            ...
            for h in reversed(history[-5:]):
                res_emoji = "✅" if h.get("status") == "TP" else "🛑"
                pnl_h     = float(h.get("pnl", 0))
                pnl_h_str = f"+{pnl_h:.2f}" if pnl_h >= 0 else f"{pnl_h:.2f}"
                lines.append(
                    f"  {res_emoji} {h['symbol']} {h['side']} "
                    f"<b>{pnl_h_str} USDT</b>"
                )
