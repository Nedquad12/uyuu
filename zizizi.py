...
class TelegramStockDataViewer:
    def __init__(self, data_folder=None):
        ...
        if data_folder:
            ...
            self.data_folder = os.path.join(os.getcwd(), "data")  # Default relative folder
    ...
    def get_data_info(self):
        ...
        return {
            ...
            'earliest_date': self.combined_df['Date'].min(),
            'latest_date': self.combined_df['Date'].max()
        }
...
You can also just send a stock code directly (e.g., type "BBCA")
    """
...
def handle_text(message):
    # Check if it's a stock code search
    text = message.text.strip().upper()
    if len(text) <= 6 and text.isalpha():  # Likely a stock code
        if viewer.combined_df is None:
            bot.reply_to(message, "❌ No data loaded. Please check if Excel files are in the 'data' folder and use /reload.")
            return
        
        # Search for stock
        stock_data = viewer.search_stock(text)
        
        if stock_data is None:
            bot.reply_to(message, f"❌ No data found for stock code: {text}")
            return
        
        # Format and send results
        formatted_data = viewer.format_stock_data(stock_data)
        response = f"📈 *Stock Data for {text}* (oldest to newest)\n\n{formatted_data}"
        
        bot.reply_to(message, response, parse_mode='Markdown')
    else:
        bot.reply_to(message, "❓ Send a stock code or use /help for available commands.")
