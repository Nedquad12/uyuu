...
class TradingIndicator:
    ...
    def format_ma_caption(self, current_price, mas, rsi_value, timeframe, atl_value, ath_value):
        ...
        caption += f"R: {format_ma_value(ath_value)}\n"
        caption += f"S: {format_ma_value(atl_value)}\n"
     ...
     # Auto-add .JK for Indonesian stocks if not already present
        ...
        for tf_name, tf_config in timeframes.items():
            ...
            try:
                ...
                ma_caption = calculator.format_ma_caption(current_price, mas_last, rsi_last, tf_name, atl_value, ath_value)
    ...
    try:
        ...
        for tf_name, tf_config in timeframes.items():
            ...
            try:
                ...
                # Kumpulkan Support (S) dan Resist (R)
                atl_value = data['Close'].min()
                ath_value = data['Close'].max()
                support_list.append(atl_value)
                resist_list.append(ath_value)
...
# Hitung average score setelah semua timeframe diproses
          ...
          if combined_scores:
            ...
            average_score = 0
            # Hitung rata-rata Support dan Resist
            avg_support = sum(support_list) / len(support_list) if support_list else 0
            avg_resist = sum(resist_list) / len(resist_list) if resist_list else 0
...
R (Resist): {avg_resist:.2f}
S (Support): {avg_support:.2f}
