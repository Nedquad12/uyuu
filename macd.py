...
def get_fundamental_data(kode_saham: str):
    """
    Ambil EPS, PBV, Revenue dan Rank sektor dari file lokal
    """
    try:
        tahun = datetime.now().year
        kuartal = "IV"  # Default tahunan

        # Path ke file LK
        path_lk = f"/home/nedquad12/database/lk/FinancialStatement-{tahun}-{kuartal}-{kode_saham}.xlsx"
        df_lk = pd.read_excel(path_lk, sheet_name=None)

        # Mata uang & pembulatan
        mata_uang, pembulatan = None, 1
        for sheet, df in df_lk.items():
            df_lower = df.applymap(lambda x: str(x).lower())
            if mata_uang is None:
                row_mu = df_lower[df_lower.apply(lambda row: row.astype(str).str.contains("mata uang pelaporan").any(), axis=1)]
                if not row_mu.empty:
                    mata_uang = row_mu.iloc[0, 1].strip().upper()
            if pembulatan == 1:
                row_pemb = df_lower[df_lower.apply(lambda row: row.astype(str).str.contains("pembulatan").any(), axis=1)]
                if not row_pemb.empty:
                    scale = row_pemb.iloc[0, 1].strip().lower()
                    pembulatan = {"million": 1e6, "billion": 1e9, "thousand": 1e3}.get(scale, 1)

        def find_value(keywords):
            for sheet, df in df_lk.items():
                df_lower = df.applymap(lambda x: str(x).lower())
                rows = df_lower[df_lower.apply(lambda row: row.astype(str).str.contains('|'.join(keywords)).any(), axis=1)]
                if not rows.empty:
                    val = rows.iloc[0, 1]
                    return float(val) * pembulatan
            return None

        ekuitas = find_value(["jumlah ekuitas", "total equity"])
        laba = find_value(["laba", "profit", "rugi", "loss"])

        # Konversi mata uang jika perlu
        if mata_uang and mata_uang != "IDR" and mata_uang != "RUPIAH":
            ticker_fx = yf.Ticker(f"{mata_uang}USD=X")
            kurs_ke_usd = ticker_fx.history(period="1d")["Close"].iloc[-1]
            ticker_usd_idr = yf.Ticker("IDR=X")
            kurs_usd_idr = ticker_usd_idr.history(period="1d")["Close"].iloc[-1]
            kurs = kurs_ke_usd * kurs_usd_idr
            ekuitas *= kurs
            laba *= kurs

        # Listed shares
        df_foreign = pd.read_excel("/home/nedquad12/database/foreign.xlsx")
        saham_beredar = df_foreign[df_foreign["Kode Saham"] == kode_saham]["Listed Shares"].values[0]

        eps = laba / saham_beredar
        pbv = ekuitas / saham_beredar

        # Rank sektor
        df_sektor = pd.read_excel("/home/nedquad12/database/sektor.xlsx")
        sektor = df_sektor[df_sektor["Kode Saham"] == kode_saham]["Sektor"].values[0]
        sektor_saham = df_sektor[df_sektor["Sektor"] == sektor]["Kode Saham"].tolist()
        eps_rank = rank_in_sector(sektor_saham, kode_saham, "EPS")
        pbv_rank = rank_in_sector(sektor_saham, kode_saham, "PBV")
        ...
        print(f"❌ Error ambil data fundamental: {e}")
        return None
...
#             Pastikan SK tidak 0 atau negatif
            ...
            kode_saham = symbol.replace(".JK", "")
            fundamental = get_fundamental_data(kode_saham)

            if fundamental:
               fundamental_text = (
              f"\n📊 *Fundamental Data*\n"
              f"• EPS: Rp{fundamental['EPS']:,.2f}\n"
              f"• PBV: {fundamental['PBV']:.2f}\n"
              f"• Revenue: Rp{fundamental['Revenue']:,.0f}\n"
              f"• Rank EPS di sektor {fundamental['Sektor']}: {fundamental['Rank EPS']}\n"
              f"• Rank PBV di sektor {fundamental['Sektor']}: {fundamental['Rank PBV']}\n"
             )
            else:
               fundamental_text = "\n⚠️ Data fundamental tidak tersedia."
...
async def cmd_lk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Command handler untuk /lk"""
    if not is_authorized_user(update):
    ...
    try:
        if not context.args or len(context.args) < 2:
            await update.message.reply_text("Gunakan: /lk SYMBOL TAHUN\nContoh: /lk AALI 2025")
            return

        kode_saham = context.args[0].upper()
        tahun = context.args[1]
        kuartal = context.args[2] if len(context.args) > 2 else "IV"
        path_lk = f"/home/nedquad12/database/lk/FinancialStatement-{tahun}-{kuartal}-{kode_saham}.xlsx"
        df_lk = pd.read_excel(path_lk, sheet_name=None)

        # Cari Mata Uang Pelaporan & Pembulatan
        mata_uang, pembulatan = None, 1
        for sheet, df in df_lk.items():
            df_lower = df.applymap(lambda x: str(x).lower())
            if mata_uang is None:
        ...
        # Cari data Ekuitas, Aset, Liabilitas, Laba (Rugi)
        def find_value(keywords):
            for sheet, df in df_lk.items():
                df_lower = df.applymap(lambda x: str(x).lower())
                rows = df_lower[df_lower.apply(lambda row: row.astype(str).str.contains('|'.join(keywords)).any(), axis=1)]
        ...
        ekuitas = find_value(["jumlah ekuitas", "total equity"])
        aset = find_value(["jumlah aset", "total assets"])
        liabilitas = find_value(["jumlah liabilitas", "total liabilities"])
        laba = find_value(["laba", "profit", "rugi", "loss"])
        ...
        if mata_uang and mata_uang != "IDR" and mata_uang != "RUPIAH":
            ticker_fx = yf.Ticker(f"{mata_uang}USD=X")
            kurs_ke_usd = ticker_fx.history(period="1d")["Close"].iloc[-1]
            ticker_usd_idr = yf.Ticker("USDIDR=X")
            kurs_usd_idr = ticker_usd_idr.history(period="1d")["Close"].iloc[-1]
            kurs = kurs_ke_usd * kurs_usd_idr
            ekuitas *= kurs
            aset *= kurs
        ...
        caption = (
            f"📊 Laporan Keuangan {kode_saham} {tahun}-{kuartal}\n\n"
            f"• Mata Uang: {mata_uang}\n"
            f"• Ekuitas: Rp{ekuitas:,.0f}\n"
            f"• Aset: Rp{aset:,.0f}\n"
            f"• Liabilitas: Rp{liabilitas:,.0f}\n"
            f"• Laba: Rp{laba:,.0f}\n\n"
            f"• EPS: Rp{eps:,.2f}\n"
            f"• PBV: {pbv:.2f}\n"
            f"• Rank EPS di sektor {sektor}: {eps_rank}\n"
            f"• Rank PBV di sektor {sektor}: {pbv_rank}\n"
        )
        await update.message.reply_text(caption)

    except Exception as e:
        await update.message.reply_text(f"❌ Error di /lk: {e}")

def rank_in_sector(saham_list, kode_saham, metric):
    # Dummy: hitung rank, implementasi penuh akan cari data riil
    return f"3 dari {len(saham_list)}"
...
def main():
    ...
    app.add_handler(CommandHandler("lk", cmd_lk))
    app.add_handler(CommandHandler("rlk", rank_in_sector))
