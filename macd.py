...
async def cmd_screener(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Command handler untuk /sc (screener IDX80 dengan skor Daily/Weekly/Monthly + pagination)"""
    user_id = update.effective_user.id
    ...
    if user_screener_results.get(user_id):
        await update.message.reply_text("📂 Menampilkan hasil screener sebelumnya dari cache...")
        await send_screener_page(update, context, user_id, page=0)
        return 
    
    user_busy_flags[user_id] = True
    await update.message.reply_text("🔍 Memulai screening IDX80... Harap tunggu beberapa menit.")

    idx80_list = [
        "ACES.JK", "ADRO.JK", "AKRA.JK", "ANTM.JK", "ARTO.JK", "ASII.JK", "BBCA.JK", "BBNI.JK", "BBRI.JK",
        "BBTN.JK", "BMRI.JK", "BRPT.JK", "BSDE.JK", "CPIN.JK", "EMTK.JK", "ERA.JK", "EXCL.JK", "GGRM.JK",
        "HMSP.JK", "ICBP.JK", "INCO.JK", "INDF.JK", "INDY.JK", "INKP.JK", "INTP.JK", "ITMG.JK", "JPFA.JK",
        "JSMR.JK", "KLBF.JK", "MDKA.JK", "MEDC.JK", "MIKA.JK", "MNCN.JK", "PGAS.JK", "PTBA.JK", "PTPP.JK",
        "SCMA.JK", "SIDO.JK", "SMGR.JK", "SMRA.JK", "TBIG.JK", "TKIM.JK", "TLKM.JK", "TOWR.JK",
        "TPIA.JK", "UNTR.JK", "UNVR.JK", "WIKA.JK", "WSKT.JK", "WTON.JK"
    ]

    results = []

    try:
        for symbol in idx80_list:
            try:
                ticker = yf.Ticker(symbol)
                timeframe_scores = {}
...
async def send_screener_page(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, page: int):
    ...
    text = f"📊 *Hasil Screener IDX80* (Halaman {page+1}/{total_pages})\n\n"
