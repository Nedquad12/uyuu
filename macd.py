...
class TradingIndicator:
    ...
    def calculate_signal_changes(self, macd_line, signal_line, k_percent, d_percent):
        ...
        for i in range(len(signal_line)):
            if i >= 1:
                ...
                if abs(change) > 15:  # 👉 hanya jika perubahan signal_line besar
                    if change > 0 and abs(macd_val) > 15:
                       mup.append(1)
                       mdn.append(0)
                    elif change < 0 and abs(macd_val) > 15:
                       mup.append(0)
                       mdn.append(-1)
                    else:
                      mup.append(0)
    ...
    def calculate_final_formula(self, data, timeframe):
        ...
        final_score = (m_gt_s*2 + mup + sup + ku1 + ku2 + ku3 - 
                      kd1 - kd2 - kd3 - mdn - sdn - m_lt_s*2 - msdn + msdu) 
...
async def cmd_calculate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
     ...
     # Auto-add .JK for Indonesian stocks if not already present
        ...
        timeframes = {
            'Daily': {'period': '1d', 'days': 50, 'download_days' : 500},
            'Weekly': {'period': '1wk', 'days': 12*7, 'download_days' : 60*7},  # 40 minggu
        ...
        for tf_name, tf_config in timeframes.items():
            try:
                ...
                if tf_name == 'Daily':
                    ...
                    display_days = 12*7    # 5 minggu untuk display
                else:  # Monthly
                    download_days = 10*30  # 3 bulan
                    display_days = 10*30
