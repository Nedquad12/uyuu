...
class TradingIndicator:
            ...
            if pd.isna(val):
                ...
                if oversold_days == 1:
                    ...
                    if macd_line.iloc[i] > 0:
                        s_values.append(1)  # Sinyal beli
                    else:
                        s_values.append(0)  # Netral
    ...
    # Hitung StdDev dari signal_line
        ...
        # Overbought area (75–100)
                ...
                oversold_days = 0
                if overbought_days == 1:
                    sup.append(-0.5 if macd_line.iloc[i] < 0 else 0)  # Hari pertama >90
                    sdn.append(-1)
                else:
                    sup.append(-1)  # Hari kedua >90
                    sdn.append(-1.5)
            else:
                sup.append(0)
                sdn.append(0)
...
async def cmd_calculate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized_user(update):
        await update.message.reply_text(
            "Hi!! Kamu tidak terdaftar di daftar member grup utama, untuk akses gratis bisa masuk ke grup https://t.me/botmsaham. "
            "Jika ingin tanpa terdaftar sebagai member grup utama hubungi @Rendanggedang atau https://x.com/saberial_link/"
        )
        return
    try:
        # Parse argumen
