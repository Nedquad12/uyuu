...
WHITELIST_USER_IDS = [
    6208519947, 5751902978, 5209950927, 1086004279
]
...
class TradingIndicator:
    ...
    def calculate_m_value(self, data, macd_line, signal_line, histogram, period=14, atr_multiplier=1.2, std_multiplier=1.5):
        """Menghitung nilai M (MACD combined) dengan threshold berdasarkan ATR"""
        # Hitung True Range
       ...
       # Hitung Std Dev MACD
        ...
        rolling_std = m_combined.rolling(window=period).std()

        # Gabungkan threshold ATR + Std Dev
        threshold = (atr * atr_multiplier) + (rolling_std * std_multiplier)
        threshold = threshold.fillna(0.01)  # hindari NaN
    ...
    def calculate_s_value(self, k_percent, d_percent):
        ...
        oversold_days = 0  # Counter hari berturut-turut oversold
        overbought_days = 0  # Counter hari berturut-turut overbought
        ...
        for val in s_combined:
            if pd.isna(val):
                ...
                oversold_days += 1
                overbought_days = 0  # reset lawannya
                if oversold_days == 1:
                    s_values.append(1)  # 👉 Hari pertama oversold = +1
                elif oversold_days == 2:
                    s_values.append(0)  # 👉 Hari kedua oversold = netral
                else:  # oversold 3 hari atau lebih
                    s_values.append(-1)  # 👉 Penalti bearish
            elif val >= 75:
               overbought_days += 1
               oversold_days = 0  # reset lawannya
               if overbought_days == 1:
                    s_values.append(1)  # 👉 Hari pertama overbought = +1
               elif overbought_days == 2:
                  s_values.append(0)  # 👉 Hari kedua overbought = netral
               else:  # overbought 3 hari atau lebih
                   s_values.append(-1)  # 👉 Penalti bearish
            else:
               s_values.append(1)  # Normal range = +1
               oversold_days = 0
               overbought_days = 0
    ...
    def calculate_signal_changes(self, macd_line, signal_line, k_percent, d_percent):
        ...
        for i in range(len(signal_line)):
            if i >= 1:
                ...
                if abs(change) > 1:  # 👉 hanya jika perubahan signal_line besar
                    if change > 0 and abs(macd_val) > 1:
                       mup.append(1)
                       mdn.append(0)
                    elif change < 0 and abs(macd_val) > 1:
                       mup.append(0)
                       mdn.append(-1)
                    else:
                      mup.append(0)
        ...
        for i in range(len(k_percent)):
            if i >= 1:
                ...
                if k_change > 0:  # Naik
                   if stoch_val > 80:
                      sup.append(-1)  # 👉 terlalu overbought
                      sdn.append(0)
                   elif stoch_val > 0:
                      sup.append(1)
                      sdn.append(0)
    ...
    def calculate_final_formula(self, data, timeframe):
        ...
        m_values = self.calculate_m_value(
           data, macd_line, signal_line, 
           period=14, 
           atr_multiplier=1.2,   # 👉 Bisa diubah sesuai preferensi
           std_multiplier=1.5    # 👉 Bisa diubah sesuai preferensi
        )
        ...
        m_gt_s, m_lt_s, msdn , msdu= self.calculate_combined_trend(m_values, s_values)
        
        # Rumus final: (m>s)+mup+sup+ku1+ku2+2*ku3-kd1-kd2-2*kd3-mdn-sdn-(m<s)-msdn
        final_score = (m_gt_s + mup + sup + ku1 + ku2 + ku3 + msdu - 
                      kd1 - kd2 - kd3 - mdn - sdn - m_lt_s - msdn) 
...
async def cmd_calculate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized_user(update):
        await update.message.reply_text(
            "Hi!! Maaf bot tidak bisa diakses, untuk akses gratis bisa masuk ke grup https://t.me/botmsaham. "
            "Jika ingin tanpa delay bisa hubungi @Rendanggedang atau https://x.com/saberial_link/"
        )
        return
    try:
        # Parse argumen
     ...
     # Auto-add .JK for Indonesian stocks if not already present
        ...
        timeframes = {
            ...
            'Monthly': {'period': '1mo', 'days': 10*30}  # 60 bulan
        }
        ...
        for tf_name, tf_config in timeframes.items():
            try:
                ...
                if tf_name == 'Daily':
                    ...
                    download_days = 25*7  # 60 minggu
                    display_days = 10*7    # 5 minggu untuk display
                else:  # Monthly
                    download_days = 11*30  # 3 bulan
                    display_days = 10*30
