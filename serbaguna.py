...
def laporan_keuangan(message):
    ...
    kode_saham = parts[1].upper() + ".JK"
    try:
        ticker = yf.Ticker(kode_saham)
        bot.reply_to(message, f"📥 Mengambil data laporan keuangan untuk {kode_saham}...")
        ...
        if income_stmt.empty or balance_sheet.empty:
            bot.reply_to(message, "❌ Tidak ada data laporan keuangan untuk saham ini.")
            return
        ...
        bv = (balance_sheet.loc['Total Assets'] - balance_sheet.loc['Total Liabilities']) / shares_outstanding
        ...
        status_eps = "📉 Murah" if harga_saat_ini < eps_fair else "📈 Mahal" if harga_saat_ini > eps_fair * 5 else "📊 Wajar"
        status_pbv = "📉 Murah" if harga_saat_ini < pbv_fair else "📈 Mahal" if harga_saat_ini > pbv_fair * 5 else "📊 Wajar"

        response = f"""📊 *Laporan Keuangan {kode_saham}*
