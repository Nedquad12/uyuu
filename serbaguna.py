...
def us_financial_report(message):
    ...
    try:
        ...
        # Hitung rasio
        if eps and eps != 0:
            pe_ratio = f"{price / eps:.2f}x EPS"
        else:
            pe_ratio = "N/A"

        if pb and pb != 0:
            pbv_ratio = f"{price / pb:.2f}x PBV"
        else:
            pbv_ratio = "N/A"
        ...
        response = (
            ...
            f"EPS: {eps if eps else 'N/A'} ({pe_ratio})\n"
            f"PBV: {pb if pb else 'N/A'} ({pbv_ratio})\n\n"
            f"📈 Revenue: {fmt(revenue)}\n"
            f"💼 Assets: {fmt(assets)}\n"
            f"📉 Liabilities: {fmt(liabilities)}\n"
            f"💰 Cashflow: {fmt(cashflow)}"
        )
