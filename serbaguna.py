...
def holdings_summary_with_stock(message):
    ...
    try:
        ...
        for month, group in df.groupby('Month'):
            ...
            grand_total = (
                ritel_total + bandar_lokal_total + bandar_asing_total +
                big_inv_lokal_total + big_inv_asing_total
            )
            # Hindari pembagian 0
            if grand_total == 0:
                grand_total = 1
            ...
            response += f"\n📅 {month.strftime('%b %Y')}\n"
            response += f"💸 Ritel: Rp {format_rupiah(ritel_total)} ({ritel_total / grand_total * 100:.1f}%)\n"
            response += f"🏦 Bandar Lokal: Rp {format_rupiah(bandar_lokal_total)} ({bandar_lokal_total / grand_total * 100:.1f}%)\n"
            response += f"🌏 Bandar Asing: Rp {format_rupiah(bandar_asing_total)} ({bandar_asing_total / grand_total * 100:.1f}%)\n"
            response += f"💼 Big Investor Lokal: Rp {format_rupiah(big_inv_lokal_total)} ({big_inv_lokal_total / grand_total * 100:.1f}%)\n"
            response += f"💼 Big Investor Asing: Rp {format_rupiah(big_inv_asing_total)} ({big_inv_asing_total / grand_total * 100:.1f}%)\n"
            response += "─" * 30 + "\n"
