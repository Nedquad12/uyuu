...
def whitelist_only(func):
    def wrapper(message):
        if message.chat.type == 'private' and message.from_user.id not in WHITELIST_USER_IDS:
            bot.send_message(
                message.chat.id,
                "Hi!! Maaf kamu tidak terdaftar sebagai member premium. Untuk gratis kunjungi https://t.me/sahamihsg. Jika ingin  bergabung menjadi member premium dan menggunakan bot sesuka hati bisa hubungi @Rendanggedang"
            )
            return
        return func(message)
    return wrapper
...
def reload_data(message):
    ...
    try:
        ...
        if isinstance(data_info['regular'], str):
            ...
            response = (
                f"✅ Bersih-bersih selesai & data berhasil dimuat ulang!\n"
                f"📊 Total pencatatan: {regular['total_records']}\n"
                f"📈 Jumlah saham: {regular['unique_codes']}\n"
                f"📅 Periode: {regular['date_range']}\n"
                f"📋 {wl_status}"
            )
...
def handle_watchlist_pagination(call):
    try:
        # Ambil cap_filter & halaman dari callback_data
        _, cap_filter, page_str = call.data.split('_')
        page = int(page_str)
        ...
        # Split jadi halaman-halaman (maks 4000 karakter)
        page_size = 4000
