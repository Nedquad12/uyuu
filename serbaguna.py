...
def queued_handler(func):
    def wrapper(message, *args, **kwargs):
        ...
        if user_id not in request_queue:
...
"""
        ...
        return f"{header}\n{summary.strip()}\n\n" + format_as_code_block(text_table)
...
@whitelist_only
@cooldown(seconds=5)
def foreign_flow_summary(message):
    log_user_activity(message.from_user.id, message.from_user.username or "Unknown", "/for")
    try:
        # ✅ Cache hanya text ranking
        if not hasattr(viewer, 'foreign_ranking_text') or viewer.foreign_ranking_text is None:
            # --- 1️⃣ Load data foreign buy/sell dari /wl ---
            wl_folder = "/home/nedquad12/database/wl"
            wl_files = sorted(glob.glob(os.path.join(wl_folder, "*.xlsx")), reverse=True)
            if not wl_files:
                bot.reply_to(message, "❌ Tidak ada data di folder wl.")
                return

            wl_dataframes = []
            for file_path in wl_files[:60]:
                try:
                    df = pd.read_excel(file_path)
                    if all(col in df.columns for col in ['Kode Saham', 'Foreign Sell', 'Foreign Buy']):
                        wl_dataframes.append(df)
                except:
                    continue
            if not wl_dataframes:
                bot.reply_to(message, "❌ Tidak ada data valid di folder wl.")
                return
            wl_combined = pd.concat(wl_dataframes, ignore_index=True)

            # Hitung total foreign buy/sell
            foreign_summary = wl_combined.groupby('Kode Saham').agg({
                'Foreign Buy': 'sum',
                'Foreign Sell': 'sum'
            }).reset_index()
            foreign_summary['Net Foreign'] = (
                foreign_summary['Foreign Buy'] - foreign_summary['Foreign Sell']
            )

            # 🧹 Hapus DataFrame mentah untuk hemat RAM
            del wl_combined, wl_dataframes
            gc.collect()

            # --- 2️⃣ Load harga penutupan dari /foreign ---
            foreign_folder = "/home/nedquad12/database/foreign"
            foreign_files = sorted(glob.glob(os.path.join(foreign_folder, "*.xlsx")), reverse=True)
            if not foreign_files:
                bot.reply_to(message, "❌ Tidak ada data di folder foreign.")
                return
            try:
                closing_df = pd.read_excel(foreign_files[0])  # Ambil hanya 1 file terbaru
                closing_df = closing_df[['Kode Saham', 'Penutupan']]  # Ambil hanya kolom Penutupan
            except:
                bot.reply_to(message, "❌ Gagal membaca data penutupan.")
                return

            # Gabungkan foreign_summary dengan penutupan
            merged_df = pd.merge(
                foreign_summary,
                closing_df,
                on='Kode Saham',
            ...
            # Hitung Net Foreign (Rp)
            merged_df['Net Foreign (Rp)'] = (
                merged_df['Net Foreign'] * merged_df['Penutupan']
            )

            # Urutkan berdasarkan Net Foreign terbesar
            ranking_df = merged_df.sort_values('Net Foreign (Rp)', ascending=False).reset_index(drop=True)
            ...
            # Buat text ranking
            lines = ["📊 *Top 20 Saham by Net Foreign (60D)*\n"]
            for i, row in enumerate(ranking_df.head(20).itertuples(), 1):
                value = row._5  # Kolom 'Net Foreign (Rp)'
                emoji = "🟢" if value > 0 else "🔴" if value < 0 else "⚪"
                lines.append(f"{i}. {row._1}: {emoji} {format_rupiah(value)}")
            viewer.foreign_ranking_text = "\n".join(lines)

            # 🧹 Bersihkan DataFrame mentah
            del foreign_summary, closing_df, merged_df, ranking_df
            gc.collect()

        parts = message.text.split()

        # ✅ Jika hanya /for → tampilkan ranking Top 20 (cached text)
        if len(parts) < 2:
            bot.reply_to(message, viewer.foreign_ranking_text, parse_mode='Markdown')
            return

        # ✅ Jika /for KODE → tampilkan detail 60D
        code = parts[1].upper()

        # Cari data untuk saham tertentu di ranking cache
        wl_folder = "/home/nedquad12/database/wl"
        wl_files = sorted(glob.glob(os.path.join(wl_folder, "*.xlsx")), reverse=True)
        dataframes = []
        for file_path in wl_files[:60]:
            try:
                df = pd.read_excel(file_path)
                if all(col in df.columns for col in ['Kode Saham', 'Foreign Buy', 'Foreign Sell']):
                    dataframes.append(df)
            except:
                continue
        if not dataframes:
        ...
        stock_data = combined_df[
            combined_df['Kode Saham'].str.upper() == code
        ]
        if stock_data.empty:
            bot.reply_to(message, f"❌ Saham {code} tidak ditemukan dalam data wl.")
            return

        # Hitung total Net Foreign
        total_net = (
            stock_data['Foreign Buy'].sum() - stock_data['Foreign Sell'].sum()
        )

        # Ambil penutupan
        foreign_folder = "/home/nedquad12/database/foreign"
        closing_df = pd.read_excel(sorted(glob.glob(os.path.join(foreign_folder, "*.xlsx")), reverse=True)[0])
        penutupan_row = closing_df[
            closing_df['Kode Saham'].str.upper() == code
        ]
        if penutupan_row.empty:
            bot.reply_to(message, f"❌ Harga penutupan untuk {code} tidak ditemukan.")
            return
        penutupan = penutupan_row.iloc[0]['Penutupan']

        # Hitung Net Foreign (Rp)
        net_foreign_rp = total_net * penutupan

        emoji = "🟢" if net_foreign_rp > 0 else "🔴" if net_foreign_rp < 0 else "⚪"
        response = (
            f"📊 Foreign Flow for {code}\n\n"
            f"{emoji} Foreign 60D: {format_rupiah(net_foreign_rp)}"
        )
        bot.reply_to(message, response)

        # 🧹 Bersihkan DataFrame mentah
        del combined_df, dataframes, closing_df
        gc.collect()

    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}")
        logger.error(f"Error in /for command: {e}")
...
def holdings_summary_with_stock(message):
    ...
    try:
        ...
        response = f"📊 Holding Summary for {code}\n💰 Harga Penutupan: Rp {format_rupiah(closing_price)}\n"

        # Simpan total bulan sebelumnya
        previous_totals = {}
        ...
        for month, group in df.groupby('Month'):
            ...
            for category, total in totals.items():
                prev_total = previous_totals.get(category, total)  # gunakan total saat ini jika tidak ada data sebelumnya
                change_pct = ((total - prev_total) / prev_total * 100) if prev_total != 0 else 0
                arrow = "🟩" if change_pct > 0 else "🟥" if change_pct < 0 else "⚪"
                response += f"{arrow} {category}: Rp {format_rupiah(total)} ({change_pct:+.1f}%)\n"

            response += "─" * 30 + "\n"
            previous_totals = totals  # update untuk bulan berikutnya
...
# 🆕 Tambahkan fungsi format_rupiah
def format_rupiah(value):
