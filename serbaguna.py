...
class TelegramStockDataViewer:
    def __init__(self, data_folder=None):
        ...
        self.lk_folder = "/home/nedquad12/database/lk"
...
@whitelist_only
def laporan_keuangan_local(message):
    log_user_activity(message.from_user.id, message.from_user.username or "Unknown", "lk")

    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ Masukan kode saham.\nContoh: `/lk AADI`", parse_mode='Markdown')
        return

    kode_input = parts[1].upper()
    LK_FOLDER = "/home/nedquad12/database/lk"
    folder_path = LK_FOLDER
    # Folder laporan keuangan

    # Cari file yang sesuai kode saham
    file_name = None
    for f in os.listdir(folder_path):
        if f.endswith(".xlsx") and f.split("-")[-1].replace(".xlsx", "") == kode_input:
            file_name = f
            break

    if not file_name:
        bot.reply_to(message, f"❌ Tidak ditemukan file laporan keuangan untuk {kode_input}.")
        return

    file_path = os.path.join(folder_path, file_name)
    try:
        # Baca sheet dengan id=3 atau nama="1210000"
        df = pd.read_excel(file_path, sheet_name="1210000", header=None)

        # Ambil data dari baris yang ditentukan
        total_assets = df.iloc[127, 1]  # CurrentYearInstant
        total_liabilities = df.iloc[246, 1]
        total_equity = df.iloc[271, 1]

        # Ambil harga saham & shares outstanding dari yfinance
        ...
        if harga_saat_ini is None or shares_outstanding is None:
            bot.reply_to(message, "❌ Tidak bisa mendapatkan harga saham atau jumlah saham beredar dari yfinance.")
            return

        # Hitung Book Value per Share (BVPS)
        bvps = total_equity / shares_outstanding
        pbv = harga_saat_ini / bvps
        pbv_fair = bvps * 2
...
📖 Total Aset: {total_assets:,.0f}
📉 Total Liabilitas: {total_liabilities:,.0f}
📈 Total Ekuitas: {total_equity:,.0f}

📖 Book Value per Saham (BVPS): {bvps:.2f}
📊 PBV: {pbv:.2f}
...
"""
        ...
        bot.reply_to(message, f"❌ Error membaca file: {str(e)}")
