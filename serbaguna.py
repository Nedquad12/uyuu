...
def holdings_summary_with_stock(message):
    ...
    try:
        ...
        df = viewer.combined_df.copy()
        df['Month'] = df['Date'].dt.to_period('M')

        # Load harga penutupan
        folder_path = viewer.chart_folder
        excel_files = sorted(glob.glob(os.path.join(folder_path, "*.xlsx")), reverse=True)
        if not excel_files:
        ...
        latest_file = excel_files[0]
        price_df = pd.read_excel(latest_file)[['Kode Saham', 'Penutupan']]
        price_df.rename(columns={'Kode Saham': 'Code'}, inplace=True)

        # Gabungkan kepemilikan + harga
        df = df.merge(price_df, on='Code', how='left')
        df.dropna(subset=['Penutupan'], inplace=True)

        # 👉 Cek apakah user memasukkan kode saham
        parts = message.text.strip().split()
        if len(parts) > 1:
            stock_code = parts[1].upper()
            df = df[df['Code'] == stock_code]
            if df.empty:
                bot.reply_to(message, f"❌ Saham {stock_code} tidak ditemukan.")
                return
            response_title = f"📊 Holdings untuk *{stock_code}*\n\n"
        else:
            response_title = "📊 *Holdings Summary (All Stocks)*\n\n"

        # Hitung nilai kepemilikan dalam Rupiah
        for col in ['Local ID', 'Local IS', 'Local PF', 'Local MF', 'Local SC',
                    'Foreign IS', 'Foreign PF', 'Foreign MF', 'Foreign SC']:
            if col in df.columns:
                df[col] = df[col] * df['Penutupan']
        ...
        summary = summary.tail(4)

        # Hitung total per kategori
        summary['Ritel'] = summary['Local ID']
        summary['Bandar Lokal'] = summary[['Local IS', 'Local PF', 'Local MF', 'Local SC']].sum(axis=1)
        summary['Bandar Asing'] = summary[['Foreign IS', 'Foreign PF', 'Foreign MF', 'Foreign SC']].sum(axis=1)

        response = response_title
        months = summary.index.tolist()

        for idx, month in enumerate(months):
            response += f"📅 *{month.strftime('%B %Y')}*\n"

            for label, col in [
                ("👥 Ritel", 'Ritel'),
                ("🏢 Bandar Lokal", 'Bandar Lokal'),
                ("🌏 Bandar Asing", 'Bandar Asing')
            ]:
                value = summary.iloc[idx][col]
                prev_value = summary.iloc[idx - 1][col] if idx > 0 else None
                change_pct = ((value - prev_value) / prev_value * 100) if prev_value else 0

                # Format angka
                if value >= 1e12:
                    formatted_value = f"Rp{value/1e12:.2f}T"
                elif value >= 1e9:
                    formatted_value = f"Rp{value/1e9:.2f}B"
                else:
                    formatted_value = f"Rp{value:,.0f}"

                delta = f"({change_pct:+.2f}%)"
                response += f"{label:<15} {formatted_value} {delta}\n"
        ...
        bot.reply_to(message, f"❌ Error di /hol: {e}")
        logger.error(f"Error di /hol: {e}")
