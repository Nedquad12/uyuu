...
def holdings_summary_with_stock(message):
    ...
    try:
        ...
        for month, group in df.groupby('Month'):
            ritel_total = group[ritel_columns].sum().sum()
            bandar_lokal_total = group[bandar_lokal_columns].sum().sum()
            bandar_asing_total = group[bandar_asing_columns].sum().sum()
            big_inv_lokal_total = group[big_investor_lokal_columns].sum().sum()
            big_inv_asing_total = group[big_investor_asing_columns].sum().sum()

            response += f"\n📅 {month.strftime('%b %Y')}\n"
            response += f"💸 Ritel: {ritel_total:,.0f}\n"
            response += f"🏦 Bandar Lokal: {bandar_lokal_total:,.0f}\n"
            response += f"🌏 Bandar Asing: {bandar_asing_total:,.0f}\n"
            response += f"💼 Big Investor Lokal: {big_inv_lokal_total:,.0f}\n"
            response += f"💼 Big Investor Asing: {big_inv_asing_total:,.0f}\n"
            response += "─" * 30 + "\n"
