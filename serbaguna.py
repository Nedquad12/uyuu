...
def laporan_keuangan(message):
    ...
    kode_yf = kode_input + ".JK"  # untuk ambil data dari Yahoo Finance
    try:
        ...
        if net_income is None or shares_outstanding == 0:
            bot.reply_to(message, f"❌ Tidak bisa menghitung EPS untuk {kode_input}. Data tidak lengkap.")
            return
        ...
        if total_assets is None or total_liabilities is None:
            bot.reply_to(message, f"❌ Tidak bisa menghitung Book Value untuk {kode_input}. Data tidak lengkap.")
            return
...
📊 QoQ Net Income: {net_income.pct_change().iloc[-1]*100:.1f}%
📊 YoY Net Income: {net_income.pct_change(periods=4).iloc[-1]*100:.1f}%
"""
