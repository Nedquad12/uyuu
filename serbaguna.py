...
# ✅ Tambahkan penanda waktu request terakhir
...
BOT_TOKEN ="7749996069:AAHIR61uOUqN5yjotelYXvKp6ZQ1H-VBgvo"
...
class TelegramStockDataViewer:
    def __init__(self, data_folder=None):
        ...
        self.blackrock_folders = {
            ...
            'us': "/home/nedquad12/database/br/us",
...
def whitelist_only(func):
    def wrapper(message):
        if message.chat.type == 'private' and message.from_user.id not in WHITELIST_USER_IDS:
            bot.send_message(
                message.chat.id,
                "Hi!! Maaf bot tidak bisa diakses, untuk akses gratis bisa masuk ke grup https://t.me/botmsaham. Jika ingin tanpa delay bisa hubungi @Rendanggedang atau https://x.com/saberial_link/"
            )
            return
        return func(message)
    return wrapper
...
def send_welcome(message):
    ...
    now = datetime.now()
    if LAST_REQUEST_TIME and (now - LAST_REQUEST_TIME).total_seconds() < 2:
        time.sleep(2)
...
@whitelist_only
def foreign_flow_table(message):
    try:
        folder_path = viewer.chart_folder  # 🆕 sudah di-init di class
        ...
        latest_file = excel_files[0]  # Ambil file terbaru
        df = pd.read_excel(latest_file)

        # Pastikan kolom ada
        required_cols = ['Kode Saham', 'Foreign Buy', 'Foreign Sell']
        if not all(col in df.columns for col in required_cols):
            bot.reply_to(message, f"❌ Kolom {required_cols} tidak ditemukan di file Excel.")
            return

        # Hitung Net Foreign
        df['Net Foreign'] = (df['Foreign Buy'] - df['Foreign Sell']) * df['Penutupan']


        # Top 20 Foreign Buy
        top_buy = df.sort_values('Net Foreign', ascending=False).head(20)
        # Top 20 Foreign Sell
        top_sell = df.sort_values('Net Foreign', ascending=True).head(20)
        ...
        top_buy['Foreign Buy'] = top_buy['Foreign Buy'].apply(format_value)
        top_sell['Foreign Sell'] = top_sell['Foreign Sell'].apply(format_value)

        # Format ke Text Table
        buy_table = "📈 *Top 20 Foreign Buy*\n\n"
        buy_table += "Kode Saham    Foreign Buy\n"
        buy_table += "\n".join([f"{row['Kode Saham']:<15}{row['Foreign Buy']:>10}" for _, row in top_buy.iterrows()])

        sell_table = "\n\n📉 *Top 20 Foreign Sell*\n\n"
        sell_table += "Kode Saham    Foreign Sell\n"
        sell_table += "\n".join([f"{row['Kode Saham']:<15}{row['Foreign Sell']:>10}" for _, row in top_sell.iterrows()])

        # Kirim ke Telegram
        bot.reply_to(message, f"{buy_table}{sell_table}", parse_mode='Markdown')
...
@whitelist_only
def laporan_keuangan_scrape(message):
    log_user_activity(message.from_user.id, message.from_user.username or "Unknown", "lk")

    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ Masukan kode saham.\nContoh: `/lk BBCA`", parse_mode='Markdown')
        return

    kode_input = parts[1].upper()
    kode_yahoo = kode_input + ".JK"
    try:
        bot.reply_to(message, f"📥 Mengambil data terbaru untuk {kode_input} dari Yahoo Finance Web...")

        # Scrape Yahoo Finance
        url = f"https://finance.yahoo.com/quote/{kode_yahoo}/financials?p={kode_yahoo}"
        headers = {"User-Agent": "Mozilla/5.0"}
        r = requests.get(url, headers=headers)
        soup = BeautifulSoup(r.text, "html.parser")

        # Extract financial table
        table = soup.find_all("div", attrs={"class": "D(tbr)"})
        if not table:
            bot.reply_to(message, f"❌ Tidak dapat menemukan data laporan keuangan untuk {kode_input}.")
            return
        ...
        for row in table[1:]:  # skip header row
            ...
            label = cols[0].text
            values = [c.text.replace(',', '').replace('-', '0') for c in cols[1:]]
            data[label] = list(map(lambda x: float(x) if x != '' else 0, values))
        ...
        # Hitung EPS dan BV
        net_income = df.loc['Net Income'].iloc[0]
        total_assets = df.loc.get('Total Assets', [0])[0]
        total_liabilities = df.loc.get('Total Liabilities', [0])[0]
        shares_outstanding = get_shares_outstanding(kode_yahoo)

        eps = net_income / shares_outstanding if shares_outstanding else 0
        bv = (total_assets - total_liabilities) / shares_outstanding if shares_outstanding else 0

        # Harga saat ini
        harga_saat_ini = get_current_price(kode_yahoo)

        # Hitung harga wajar
        eps_fair = eps * 20
        pbv_fair = bv * 2

        status_eps = "📉 Murah" if harga_saat_ini < eps_fair else "📈 Sangat Mahal" if harga_saat_ini > eps_fair * 5 else "📊 Wajar"
        status_pbv = "📉 Murah" if harga_saat_ini < pbv_fair else "📈 Sangat Mahal" if harga_saat_ini > pbv_fair * 5 else "📊 Wajar"

        response = f"""📊 *Laporan Keuangan {kode_input}*

💵 Harga Sekarang: {harga_saat_ini:,.2f}
📈 EPS (TTM): {eps:.2f}
🎯 Harga Wajar EPS (20x): {eps_fair:,.2f} → {status_eps}

📖 Book Value per Saham: {bv:.2f}
🎯 Harga Wajar PBV (2x): {pbv_fair:,.2f} → {status_pbv}
"""
        bot.reply_to(message, response, parse_mode="Markdown")

    except Exception as e:
        bot.reply_to(message, f"❌ Error scraping Yahoo Finance: {str(e)}")
...
def get_current_price(ticker):
    ...
    return float(price_tag.text.replace(',', '')) if price_tag else 0


def get_shares_outstanding(ticker):
    """Scrape shares outstanding from Yahoo Finance"""
    url = f"https://finance.yahoo.com/quote/{ticker}/key-statistics?p={ticker}"
    headers = {"User-Agent": "Mozilla/5.0"}
    r = requests.get(url, headers=headers)
    soup = BeautifulSoup(r.text, "html.parser")
    shares_tag = soup.find("span", text="Shares Outstanding").find_next("td")
    shares_text = shares_tag.text.replace('B', '').replace('M', '')
    if 'B' in shares_tag.text:
        return float(shares_text) * 1e9
    elif 'M' in shares_tag.text:
        return float(shares_text) * 1e6
    return 0
