...
def holdings_summary_with_stock(message):
    ...
    try:
        ...
        for month, group in df.groupby('Month'):
            ...
            response += f"\n📅 {month.strftime('%b %Y')}\n"
            response += f"💸 Ritel: Rp {format_rupiah(ritel_total)}\n"
            response += f"🏦 Bandar Lokal: Rp {format_rupiah(bandar_lokal_total)}\n"
            response += f"🌏 Bandar Asing: Rp {format_rupiah(bandar_asing_total)}\n"
            response += f"💼 Big Investor Lokal: Rp {format_rupiah(big_inv_lokal_total)}\n"
            response += f"💼 Big Investor Asing: Rp {format_rupiah(big_inv_asing_total)}\n"
            response += "─" * 30 + "\n"
