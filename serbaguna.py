...
Perintah saham USA
/ikus - Laporan keuangan saham USA
...
@whitelist_only
def us_financial_report(message):
    log_user_activity(message.from_user.id, message.from_user.username or "Unknown", "lkus")
    ...
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ Masukan ticker saham.\nContoh: `/lkus TSLA`", parse_mode='Markdown')
        return

    ticker_symbol = parts[1].upper()
    try:
        ticker = yf.Ticker(ticker_symbol)
        info = ticker.info

        if 'regularMarketPrice' not in info or info['regularMarketPrice'] is None:
            bot.reply_to(message, f"❌ Tidak bisa mendapatkan harga untuk {ticker_symbol}")
            return
        ...
        liabilities = info.get('totalLiab', 'N/A')
        revenue = info.get('totalRevenue', 'N/A')
        cashflow = info.get('operatingCashflow', 'N/A')

        # Hitung rasio
        pe_ratio = f"{price/eps:.2f}x EPS" if eps else "N/A"
        pbv_ratio = f"{price/pb:.2f}x PBV" if pb else "N/A"

        response = (
            f"📊 *Laporan Keuangan* {ticker_symbol}\n"
            f"💵 Harga Sekarang: ${price:,.2f}\n"
            f"EPS: {eps:.2f} ({pe_ratio})\n"
            f"PBV: {pb:.2f} ({pbv_ratio})\n\n"
            f"📈 Revenue: ${revenue:,}\n"
            f"💼 Assets: ${assets:,}\n"
        ...
        logger.error(f"Error in /lkus command: {e}")
