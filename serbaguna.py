...
def foreign_flow_summary(message):
    ...
    try:
        # ✅ Load & proses hanya jika belum ada cache text
        if not hasattr(viewer, 'foreign_ranking_text') or viewer.foreign_ranking_text is None:
            folder_path = "/home/nedquad12/database/foreign"
            excel_files = sorted(glob.glob(os.path.join(folder_path, "*.xlsx")), reverse=True)
            if not excel_files:
                bot.reply_to(message, "❌ Tidak ada data foreign di folder.")
                return

            # Gabungkan max 60 hari data
            dataframes = []
            for file_path in excel_files[:60]:
                df = pd.read_excel(file_path)
                df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
                dataframes.append(df)
            combined_df = pd.concat(dataframes, ignore_index=True)

            # Hitung Net Foreign (dalam Rupiah)
            combined_df['Net Foreign (Rp)'] = (
                (combined_df['Foreign Buy'] - combined_df['Foreign Sell']) * combined_df['Penutupan']
            )

            # Group & sort total 60 hari
            ranking_df = combined_df.groupby('Kode Saham').agg({
                'Net Foreign (Rp)': 'sum'
            }).sort_values('Net Foreign (Rp)', ascending=False).reset_index()

            # Format ranking ke text
        ...
        # ✅ Jika /for KODE → tampilkan detail 60D & hari ini
        code = parts[1].upper()
        folder_path = "/home/nedquad12/database/foreign"
        excel_files = sorted(glob.glob(os.path.join(folder_path, "*.xlsx")), reverse=True)
        dataframes = []
        ...
        if stock_data.empty:
            bot.reply_to(message, f"❌ Saham {code} tidak ditemukan dalam data foreign.")
            return

        # Hitung detail
        stock_data['Net Foreign (Rp)'] = (
            (stock_data['Foreign Buy'] - stock_data['Foreign Sell']) * stock_data['Penutupan']
        )
        now_foreign = stock_data.iloc[0]['Net Foreign (Rp)']
        total_60d = stock_data['Net Foreign (Rp)'].sum()

        emoji_now = "🟢" if now_foreign > 0 else "🔴" if now_foreign < 0 else "⚪"
        emoji_60d = "🟢" if total_60d > 0 else "🔴" if total_60d < 0 else "⚪"

        response = (
            f"📊 Foreign Flow for {code}\n\n"
            f"{emoji_now} Now Foreign: {format_rupiah(now_foreign)}\n"
            f"{emoji_60d} Foreign 60D: {format_rupiah(total_60d)}"
        )
        bot.reply_to(message, response)
