...
Perintah saham USA
/lkus - Laporan keuangan saham USA
...
@bot.message_handler(commands=['lkus'])
@whitelist_only
def us_financial_report_investpy(message):
    log_user_activity(message.from_user.id, message.from_user.username or "Unknown", "lkus_investpy")
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ Masukkan ticker saham.\nContoh: `/lkus TSLA`", parse_mode='Markdown')
        return
    ...
    try:
        # Cari
        search = investpy.search_quotes(text=ticker, products=['stocks'], countries=[country], n_results=1)
        info = search.retrieve_information()
        recent = search.retrieve_recent_data()

        price = info.get('prevClose') or recent['Close'][-1]
        eps = info.get('eps')
        revenue = info.get('revenue')
        pb = info.get('ratio')  # P/E ratio
        # untuk PBV perlu scraping lagi; investpy tidak langsung sediakan priceToBook
        # Alternatif: balance sheet digunakan alpha vantage

        # Financial summary
        is_ = investpy.get_stock_financial_summary(stock=ticker, country=country, summary_type='income_statement', period='annual')
        bs = investpy.get_stock_financial_summary(stock=ticker, country=country, summary_type='balance_sheet', period='annual')
        cf = investpy.get_stock_financial_summary(stock=ticker, country=country, summary_type='cash_flow_statement', period='annual')

        def latest(df, field):
            return df.loc[field].iloc[0] if field in df.index else None

        total_revenue = latest(is_, 'Total Revenue')
        total_assets = latest(bs, 'Total Assets')
        total_liab = latest(bs, 'Total Liabilities') or latest(bs, 'Total Liab')
        operating_cf = latest(cf, 'Operating Cash Flow') or latest(cf, 'Net Cash from Operating Activities')

        pe_ratio_calc = f"{price / eps:.2f}x EPS" if eps else "N/A"
        pbv_ratio = "N/A"  # PBV belum tersedia

        def fmt(x):
            return f"${x:,.0f}" if isinstance(x, (int, float)) else "N/A"

        resp = (
            f"📊 *Laporan Keuangan* {ticker}\n"
            f"💵 Harga Sekarang: ${price:,.2f}\n"
            f"EPS: {eps or 'N/A'} ({pe_ratio_calc})\n"
            f"PBV: {pb or 'N/A'} ({pbv_ratio})\n\n"
            f"📈 Revenue: {fmt(total_revenue)}\n"
            f"💼 Assets: {fmt(total_assets)}\n"
            f"📉 Liabilities: {fmt(total_liab)}\n"
            f"💰 Cashflow Operasional: {fmt(operating_cf)}"
        )
