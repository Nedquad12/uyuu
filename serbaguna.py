...
def laporan_keuangan_scrape(message):
    ...
    try:
        bot.reply_to(message, f"📥 Mengambil data terbaru untuk {kode_input} dari Yahoo Finance...")

        # Ambil data laporan keuangan dari API Yahoo
        url = f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/{kode_yahoo}?modules=incomeStatementHistoryQuarterly,balanceSheetHistoryQuarterly,cashflowStatementHistoryQuarterly"
        headers = {"User-Agent": "Mozilla/5.0"}
        res = requests.get(url, headers=headers)
        if res.status_code != 200:
            bot.reply_to(message, f"❌ Gagal mengambil data Yahoo Finance: {res.status_code}")
            return
        ...
        # Income Statement
        income = result['incomeStatementHistoryQuarterly']['incomeStatementHistory']
        net_income = income[0]['netIncome']['raw']

        # Balance Sheet
        balance = result['balanceSheetHistoryQuarterly']['balanceSheetStatements']
        total_assets = balance[0]['totalAssets']['raw']
        total_liabilities = balance[0]['totalLiab']['raw']

        # Ambil harga saham & shares outstanding
        quote_url = f"https://query2.finance.yahoo.com/v7/finance/quote?symbols={kode_yahoo}"
        quote_res = requests.get(quote_url, headers=headers)
        quote_data = quote_res.json()
        current_price = quote_data['quoteResponse']['result'][0]['regularMarketPrice']
        shares_outstanding = quote_data['quoteResponse']['result'][0]['sharesOutstanding']

        # Hitung EPS dan Book Value
        eps = net_income / shares_outstanding
        bv = (total_assets - total_liabilities) / shares_outstanding

        # Hitung harga wajar
        eps_fair = eps * 20
        pbv_fair = bv * 2

        status_eps = "📉 Murah" if current_price < eps_fair else "📈 Sangat Mahal" if current_price > eps_fair * 5 else "📊 Wajar"
        status_pbv = "📉 Murah" if current_price < pbv_fair else "📈 Sangat Mahal" if current_price > pbv_fair * 5 else "📊 Wajar"
