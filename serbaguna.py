...
def laporan_keuangan(message):
    ...
    kode_yf = kode_input + ".JK"  # Internal fetch
    try:
        ticker = yf.Ticker(kode_yf)
        bot.reply_to(message, f"📥 Mengambil data laporan keuangan untuk {kode_input}...")

        # Ambil laporan keuangan (kuartalan, 2 tahun terakhir)
        income_stmt = ticker.quarterly_financials
        balance_sheet = ticker.quarterly_balance_sheet

        if income_stmt.empty:
            bot.reply_to(message, f"❌ Tidak ada data laporan keuangan untuk {kode_input}.")
            return

        shares_outstanding = ticker.info.get('sharesOutstanding', 0)
        if shares_outstanding == 0:
            bot.reply_to(message, f"❌ Data jumlah saham {kode_input} tidak ditemukan.")
            return
        ...
        for key in ['Net Income', 'NetIncome']:
            if key in income_stmt.index:
                net_income = income_stmt.loc[key]
                break
        if net_income is None:
            bot.reply_to(message, f"❌ Tidak bisa menghitung EPS untuk {kode_input}. 'Net Income' tidak ada.")
            return
        eps = (net_income / shares_outstanding).dropna()

        # Harga saat ini
        harga_saat_ini = ticker.info.get('currentPrice', None)
        if harga_saat_ini is None:
            bot.reply_to(message, f"❌ Tidak bisa mendapatkan harga saham {kode_input} saat ini.")
            return
        ...
        for key in ['Total Assets', 'TotalAssets']:
            if key in balance_sheet.index:
                ...
                break
        for key in ['Total Liabilities', 'Total Liab', 'Tot Liabilities', 'Liabilities']:
            if key in balance_sheet.index:
                total_liabilities = balance_sheet.loc[key]
                break

        bv_available = total_assets is not None and total_liabilities is not None
        if bv_available:
            bv = ((total_assets - total_liabilities) / shares_outstanding).dropna()
            pbv_fair = bv[-1] * 2
            status_pbv = "📉 Murah" if harga_saat_ini < pbv_fair else "📈 Sangat Mahal" if harga_saat_ini > pbv_fair * 5 else "📊 Wajar"
        else:
            bv = None

        # Hitung harga wajar EPS
        eps_fair = eps[-1] * 20
...
"""
        ...
        if bv_available:
            response += f"""
📖 Book Value per Saham: {bv[-1]:.2f}
🎯 Harga Wajar PBV (2x): {pbv_fair:,.2f} → {status_pbv}
"""
        else:
            response += "\n⚠️ *Book Value tidak tersedia. Data Total Assets / Liabilities hilang.*\n"

        # Tambahkan QoQ & YoY
        qoq_change = net_income.pct_change().iloc[-1] * 100
        yoy_change = net_income.pct_change(periods=4).iloc[-1] * 100

        response += f"""
📅 Data Kuartal Terakhir: {income_stmt.columns[-1].strftime('%b %Y')}
📊 QoQ Net Income: {qoq_change:+.1f}%
📊 YoY Net Income: {yoy_change:+.1f}%
"""

        bot.reply_to(message, response, parse_mode="Markdown")

    except Exception as e:
        bot.reply_to(message, f"❌ Error mengambil data: {str(e)}")
